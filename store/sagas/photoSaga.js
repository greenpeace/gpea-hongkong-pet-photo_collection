...
export function* getPhoto() {
  try {
    ...
    const getPhotoTotal = yield call(() => axios.get(`${process.env.G_SHEET}/photo-collection?limit=1`)
    .then((response) => {      
    ...
    for (let i = 0; i < getPhotoTotal; i+=100) {
      fetchURLs = [...fetchURLs, `${process.env.G_SHEET}/photo-collection?offset=${i}`]
    }

    const allPhoto = yield call(() => axios.all(
