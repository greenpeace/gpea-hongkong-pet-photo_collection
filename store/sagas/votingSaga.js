...
export function* getVoting() {
  try {
    let fetchURLs = [];
    let voteResult = [];
    const getVotingTotal = yield call(() =>
      axios
        .get(`${process.env.G_SHEET}/photo-collection-voting?limit=1`)
        .then((response) => {
    ...
    for (let i = 0; i < getVotingTotal; i += 100) {
      fetchURLs = [
        ...fetchURLs,
        `${process.env.G_SHEET}/photo-collection-voting?offset=${i}`,
      ];
    }

    const allVoting = yield call(() =>
      axios
    ...
    const merged = yield _.merge(
      _.keyBy(photoData, 'id'),
      _.keyBy(count, 'id')
    );
    const values = yield _.values(merged)
...
export function* addVoting(action) {
  try {
    const addVotingResult = yield call(() =>
      axios
